    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in-0 duration-300">
      <div className="relative bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg h-auto overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center p-8">
            <div className="w-full max-w-sm h-32 border-4 border-white/50 rounded-lg"></div>
...
export const Dashboard: React.FC = () => {
  ...
  const saveLog = (log: DailyLogEntry[]) => {
      ...
      try {
        ...
        console.error("Failed to save log data to local storage", err);
      }
      setDailyLog(log);
  }
  ...
  const handleEditFormChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setEditingItem({
          ...
          data: {
              ...editingItem.data,
              [name]: name === 'foodName' ? value : (value === '' ? 0 : parseFloat(value))
          }
      });
  };
        ...
        <div className="grid grid-cols-2 gap-4 text-center">
            ...
            <div className="flex flex-col items-center justify-center">
                ...
                <div className="flex items-center gap-4">
                    <button onClick={() => handleWaterChange(-1)} disabled={waterIntake <= 0} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 disabled:opacity-50 transition-colors" aria-label="Remove one glass of water">
                        <MinusCircle size={28} />
                    </button>
                    <button onClick={() => handleWaterChange(1)} className="text-blue-500 hover:text-blue-700 transition-colors" aria-label="Add one glass of water">
                        <PlusCircle size={28} />
                    </button>
                </div>
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg">
        ...
        {logMode === 'snap' && (
            <div className="space-y-4">
                <div className="border-2 border-dashed border-teal-300 dark:border-teal-700 rounded-xl p-8 text-center cursor-pointer hover:bg-teal-100/60 dark:hover:bg-gray-700/50 transition-colors" onClick={() => fileInputRef.current?.click()}>
                 ...
                 {manualItems.map((item, index) => (
                    <div key={index} className="p-4 rounded-lg border border-teal-100 dark:border-teal-800 bg-teal-50/50 dark:bg-gray-700/30 space-y-3">
                        <div className="flex items-center gap-2">
                             <input type="text" placeholder="Food Name" value={item.foodName || ''} onChange={(e) => handleManualItemChange(index, 'foodName', e.target.value)} className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-teal-400 focus:outline-none bg-white dark:bg-gray-800"/>
                             <button onClick={() => removeManualItem(index)} disabled={manualItems.length <= 1} className="text-gray-400 hover:text-red-500 disabled:opacity-50"><Trash2 size={20}/></button>
                        </div>
        ...
        {logMode === 'activity' && (
             <div className="space-y-4 animate-in fade-in-0 duration-300">
                {activityItems.map((item, index) => (
                    <div key={index} className="p-4 rounded-lg border border-teal-100 dark:border-teal-800 bg-teal-50/50 dark:bg-gray-700/30 space-y-3">
                        <div className="flex items-center gap-2">
                             <input type="text" list="common-activities" placeholder="Select or type an activity..." value={item.name} onChange={(e) => handleActivityItemChange(index, 'name', e.target.value)} className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-teal-400 focus:outline-none bg-white dark:bg-gray-800"/>
                             <datalist id="common-activities">
                                {COMMON_ACTIVITIES.map((act) => <option key={act.name} value={act.name} />)}
                            </datalist>
                             <button onClick={() => removeActivityItem(index)} disabled={activityItems.length <= 1} className="text-gray-400 hover:text-red-500 disabled:opacity-50"><Trash2 size={20}/></button>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                             <input type="number" min="0" placeholder="Duration (min)" value={item.duration} onChange={(e) => handleActivityItemChange(index, 'duration', e.target.value)} className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-teal-400 focus:outline-none bg-white dark:bg-gray-800"/>
                             <input type="number" min="0" placeholder="Calories Burned" value={item.caloriesBurned} onChange={(e) => handleActivityItemChange(index, 'caloriesBurned', e.target.value)} className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-teal-400 focus:outline-none bg-white dark:bg-gray-800"/>
                        </div>
                    </div>
                ))}

...
if (!API_KEY) {
  throw new Error("VITE_API_KEY environment variable is not set. Please create a .env file and add it.");
}
...
`;
    ...
    const recipesWithImages = await Promise.all(
        parsedRecipes.map(async (recipe: any) => {
            const imageQuery = recipe.imageSearchQuery || recipe.title;
            if (imageQuery) {
              const imageUrl = await generateRecipeImage(imageQuery);
              return { ...recipe, imageUrl };
            }
            return { ...recipe, imageUrl: `https://source.unsplash.com/800x600/?food` };
        })
    );
